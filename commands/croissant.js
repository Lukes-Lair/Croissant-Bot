...
module.exports = {
      ...
      subcommand
        ...
        .addStringOption((string) =>
          string
            .setName("name")
            .setDescription(
              "Name of the Emoji ( Will display in users dropdown)"
            )
            .setRequired(true)
        )
    )
  ...
  async execute(interaction) {
    ...
    const isTrusted = interaction.memberPermissions
      .toArray()
      .includes("ManageEmojisAndStickers");
    if (subcommand === "stats") {
      const member = interaction.options.getUser("member");
      const emojiText = interaction.options.getString("emoji-name");
      const emojiDB = await CroissantEmojiDB.findOne({
        name: emojiText,
        guildID: guild,
      }).lean();
      ...
      const allEmoji = await CroissantMessagesDB.find({
        name: emojiDB.name,
        guildID: guild,
      })
        .sort({ count: -1 })
        .lean();
      const ranking = allEmoji.findIndex(
        (entry) =>
          entry.userID === (member == null ? interaction.user.id : member.id)
      );
      const result = await CroissantMessagesDB.findOne({
        userID: member == null ? interaction.user.id : member.id,
        name: `${emojiDB.name}`,
        guildID: guild,
      });
      ...
      if (result) {
        ...
        interaction.reply({
          content: ` ${
            member == null ? interaction.user : `<@${member.id}>`
          } has sent ${result.count} ${
            emojiDB.emoji
          } since ${date} and is ranked #${ranking + 1} in ${emojiDB.emoji}`,
        });
      } else {
        interaction.reply(
          ` ${
            member == null ? interaction.user : `<@!${member.id}>`
          } has not sent any ${emojiDB.emoji}`
        );
      }
      ...
      await interaction.reply({
        content: `Successfully saved ${emojiID} with text \`${emojiText}\` to the database!`,
      });
      ...
      const deleted = await CroissantMessagesDB.deleteMany({
        guildID: guild,
        name: entry.name,
      });
      if (!entry) {
      ...
      interaction.reply(
        `Successfully deleted \`${emojiName}\` from the database`
      );
    } else if (subcommand === "leaderboard") {
      const emojiName = interaction.options.getString("emoji-name");
      if (emojiName === null) {
        const top10 = await CroissantMessagesDB.find({ guildID: guild })
          .sort({ count: -1 })
          .limit(10)
          .lean();
        ...
        for (let i = 0; i < top10.length; i++) {
          ...
          const type = await CroissantEmojiDB.findOne({
            name: top10[i].name,
            guildID: top10[i].guildID,
          });
          if (i === 0) {
            message += `<:gold:1393301072122220575> ${
              i + 1
            }. <@!${userID}> — ${count} ${type.emoji}\n`;
          } else if (i === 1) {
            message += `<:silver:1393301087494209556> ${
              i + 1
            }. <@!${userID}> — ${count} ${type.emoji}\n`;
          } else if (i === 2) {
            message += `<:bronze:1393301100307939460> ${
              i + 1
            }. <@!${userID}> — ${count} ${type.emoji}\n`;
          } else {
        ...
        const emojiEntry = await CroissantEmojiDB.findOne({
          name: emojiName,
          guildID: guild,
        });
        ...
        const sorted = await CroissantMessagesDB.find({
          name: emojiname,
          guildID: guild,
        })
          .sort({ count: -1 })
        ...
        if (sorted.length === 0) {
          return interaction.reply({
            content: "No data found for that emoji.",
            ephemeral: true,
          });
        }
